...
function recursos() {
    $salida = "
        ...
        <form enctype=\"multipart/form-data\" method=\"post\" action=\"insert/addRecursos.php\">
            ...
            $salida = $salida."<div class=\"form-group\">
                ...
                if (isset($_SESSION["u_leccion"])) {
                    $nom_leccion = $_SESSION["u_leccion"];
                    $sqlLeccion = "SELECT clave FROM Lecciones WHERE nombre = '".$nom_leccion."'";
                    $sqlLecciones = mysql_query($sqlLeccion);
                    $rowLecc = mysql_fetch_array($sqlLecciones);
                    $id_leccion = $rowLecc['clave'];
                    $hay_Leccion = 1;
                }
                if ($hay_Leccion == 1) {
                    $sqlTema = "SELECT * FROM Temas WHERE id_leccion = ".$id_leccion;
                }else{
                    $sqlTema = "SELECT * FROM Temas";
                }
                
                $sql= mysql_query($sqlTema);
                if ($sql == false) {
        ...
        <form method=\"post\" action=\"insert/addLecciones.php\">
            ...
            require("connect_db.php");
                if (isset($_SESSION["u_curso"])) {
                    $cve_curso = $_SESSION["u_curso"];
                    $sqlQ= "SELECT * FROM Cursos where clave = '".$cve_curso."'";
                }else{
                    $sqlQ= "SELECT * FROM Cursos";
                }
...
function temas() {
    $salida = "
        ...
        <form method=\"post\" action=\"insert/addTemas.php\">
            ...
            if (isset($_SESSION["u_leccion"])) {
                    $cve_curso = $_SESSION["u_curso"];
                    $sqlQ= "SELECT * FROM Lecciones where clave_curso = '".$cve_curso."'";
                }else{
                    $sqlQ= "SELECT * FROM Lecciones";
                }
                $sql= mysql_query($sqlQ);
                if ($sql == false) {
